...
async def load_new_transactions_from_bill(
    ...
    while True:
        ...
        for loader_cls in loaders:
            loader: Loader = loader_cls(
                account_rules_list, classify_rules_list
            )
        ...
        if new_transactions:
            ...
            event.set()
            insert_transactions(
                list(new_transactions.values()), TranStatus.Raw
            )
            dump_db(args.output_path)
    ...
    while True:
        ...
        for _, content in contents.items():
            ...
            if confirmed:
                ...
                dump_db(args.output_path)
                logger.show(
                    "[transactions confirmed][count=%s]" % len(confirmed)
                )
...
async def finial_adb_output(args: "BillingArgs", queue: asyncio.Queue) -> None:
    ...
    while True:
        ...
        await asyncio.sleep(adb_interval)
...
async def main() -> None:
    ...
    coroutines = []
    coroutines.append(
        load_new_transactions_from_bill(args, unconfirmed_transaction_event)
    )
    coroutines.append(
        output_confirmed_data(args, unconfirmed_transaction_event)
    )
    uninsert_transaction_list: asyncio.Queue = asyncio.Queue()
    coroutines.append(handle_confirmed_data(args, uninsert_transaction_list))
    coroutines.append(finial_adb_output(args, uninsert_transaction_list))
